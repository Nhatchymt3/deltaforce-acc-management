...
import { createSource, updateSource, deleteSource } from '@/app/actions/sources';
...
export function SourceManager({ initialSources }: { initialSources: Source[] }) {
  ...
  async function handleSaveEdit(id: string) {
    ...
    setLoading(`edit-${id}`);
    try {
      const updated = await updateSource(id, trimmed);
      setSources((prev) =>
                ...
                {editingId === source.id ? (
                  <div className="flex flex-1 items-center gap-2">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') void handleSaveEdit(source.id);
      ...
      {deletingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-sm rounded-xl border border-white/[0.08] bg-card p-5 shadow-2xl space-y-4">
            <h3 className="font-display font-semibold text-foreground text-base">Xóa nguồn?</h3>
